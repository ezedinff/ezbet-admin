...
const Filter = (props:
     ...
     formData?: {intialValues?: any, selectValues?: any}},
      ...
      const getFormElements = (elements: {name: string; label: string; type: FormType}[]) => {
            ...
            switch (element.type) {
                ...
                case FormType.NUMBER:
                    return (
                        <Col className="gutter-row" md={5}>
                            <Form.Item name={element.name} label={element.label}>
                                <InputNumber placeholder={element.label} style={{minWidth: '100%'}}/>
                            </Form.Item>
                        </Col>
                    );
                    ...
                    return (
                        <Col className="gutter-row" md={5}>
                         <Form.Item name={element.name} label={element.label}>
                                ...
                                >
                                    {
                                       props.formData && getOptions(props.formData.selectValues[element.name])
                                    }
                                </Select>
                         </Form.Item>
                        </Col>
    ...
    const getOptions = (options: {label: string, value: any[]}[]) => {
        return options.map((v: {label: string; value: any}) => <Select.Option key={v.label} value={v.value}>{v.label}</Select.Option>);
    }
    return (
        <Form
        {...layout}
        name="filter"
        initialValues={props.formData && props.formData.intialValues ? props.formData.intialValues: {}}
        onFinish={onFinish}
